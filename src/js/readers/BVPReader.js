...
function decompressS3DC(src, MMV, dimensions) {
    ...
    for (const val of src) {
        const index_first = (val & (3 << 6)) >>> 6;
        const index_second = (val & (3 << 4)) >>> 4;
        const index_third = (val & (3 << 2)) >>> 2;
        const index_fourth = val & 3;
        const dv_first =
            min + (max - min) * (index_first / (Math.pow(2, ibits) - 1));
        const dv_second =
            min + (max - min) * (index_second / (Math.pow(2, ibits) - 1));
        const dv_third =
            min + (max - min) * (index_third / (Math.pow(2, ibits) - 1));
        const dv_fourth =
            min + (max - min) * (index_fourth / (Math.pow(2, ibits) - 1));
        dst[position] = dv_first;
        dst[position + 1] = dv_second;
        dst[position + 2] = dv_third;
        dst[position + 3] = dv_fourth;
        position += 4;
    }
    return dst;
}
