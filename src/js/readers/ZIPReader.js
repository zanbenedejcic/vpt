...
class ZIPReader extends AbstractReader {
    ...
    _readCD(handlers) {
        ...
        const readCD = function () {
            this._loader.readData(
                this._eocd.offset,
                this._eocd.offset + this._eocd.size,
                {
                    onData: (data) => {
                        data = new DataView(data);
                        let offset = 0;
                        ...
                        for (let i = 0; i < this._eocd.entries; i++) {
                            ...
                            const compressedSize = data.getUint32(
                                offset + 20,
                                true
                            );
                            const uncompressedSize = data.getUint32(
                                offset + 24,
                                true
                            );
                            const fileNameLength = data.getUint16(
                                offset + 28,
                                true
                            );
                            const extraFieldLength = data.getUint16(
                                offset + 30,
                                true
                            );
                            const fileCommentLength = data.getUint16(
                                offset + 32,
                                true
                            );
                            const cdEntrySize =
                            ...
                            const headerOffset = data.getUint32(
                                offset + 42,
                                true
                            );
                            entries[i] = {
